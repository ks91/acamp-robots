...
- Use `.venv/bin/acamp-robot call walk DIRECTION DURATION` for forward, backward, left, and right. Valid names are `forward`, `backward`, `left`, and `right`; requested walks may last up to 30 seconds as one smooth server-timed action. The default step is 15, not the ineffective five-unit step. An explicitly requested step may be 1–30.
...
- Use `.venv/bin/acamp-robot call turn_by DIRECTION DEGREES` when a member requests a relative angle, such as `180度時計回りに回転して` or `真後ろを向いて`. It integrates the MPU6050 Z-axis gyro, slows near the target, and stops on success or an angle-scaled internal deadline. Do not translate degrees into the low-level gait-angle parameter and do not split a requested turn into visibly separate actions.
...
- Honor requested walking and turning durations up to the API's 30-second bound as one smooth action. Do not expose the internal stop deadline as an arbitrary five-second participant limit or split a ten-second request into stop-start chunks.
...
- The RPC surface also provides a single ultrasonic distance (`sonic`), a robust multi-sample distance summary (`distance_read`), battery voltage (`power`), static RGB color (`led_color`), LED effects (`led_mode`), buzzer control, camera capture, red-ball tracking, head and whole-body posture control, balance, and per-leg position and angle control. `power` names the first vendor ADC value `servo_voltage_v` and the second `raspberry_pi_voltage_v`; never reverse or guess these channels. `raw_voltages` preserves the original pair in that same order. Prefer `distance_read` for measured experiments and `sonic` when latency matters. Query `capabilities` rather than guessing method names.
